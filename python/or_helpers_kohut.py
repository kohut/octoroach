...
from callbackFunc_kohut import xbee_received
import datetime
import serial
import shared
...
class Robot:
    ...
    def setHallGains(self, gains, retries = 8):
        tries = 1
        self.hallGains = gains
        while not(self.hall_gains_set) and (tries <= retries):
            self.clAnnounce()
            print "Setting hall motor gains...   ",tries,"/8"
            self.tx( 0, command.SET_HALL_GAINS, pack('10h',*gains))
            tries = tries + 1
            time.sleep(0.3)
    ...
    def setVelProfile(self, params):
        ...
        self.tx(0, command.SET_VEL_PROFILE, pack('24h',*temp))
        #time.sleep(0.3)
...
def verifyAllHallGainsSet():
    #Verify all robots have motor gains set
    for r in shared.ROBOTS:
        if not(r.hall_gains_set):
            print "CRITICAL : Could not SET HALL GAINS on robot 0x%02X" % r.DEST_ADDR_int
            xb_safe_exit()
